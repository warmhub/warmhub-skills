...
- **Arc** for directional 2-way relationships ("A blocks B" — A and B are different roles).
- **Bond** for symmetric 2-way relationships ("A and B are duplicates of each other" — order doesn't matter).
- **Set** for symmetric or mechanical groups with more than two members.
...
If step 3 (`about` arity) is producing nonsense answers — e.g., a relationship has more endpoints than Arc, Bond, or Set can express — pause and read [primitives.md § Collections](../../modeling-foundations/references/primitives.md#collections--arc-bond-set-list) carefully. The public collection types cover most cases; if none fits, you likely have a genuine 3-or-more-way relation that wants a named domain shape/assertion rather than a collection primitive, or a multi-relationship situation that decomposes into multiple assertion shapes.
